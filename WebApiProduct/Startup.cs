        ...
        public void ConfigureServices(IServiceCollection services)
        {
            ...
            services.AddSwaggerGen(c =>
            {
                typeof(ApiVersionCommon).GetEnumNames().ToList().ForEach(v=> {
                    c.SwaggerDoc($"{v}", new OpenApiInfo { Title = "WebApiProduct", Version = $"{v}", Description = "restful架构下的产品服务" });
                });

                //c.SwaggerDoc("v1", new OpenApiInfo { Title = "WebApiProduct", Version = "v1", Description = "restful架构下的产品服务" });
            });
            #region JWT鉴权授权
